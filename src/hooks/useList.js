...
export const useList = (initialParams = {}) => {
  ...
  const updateState = useCallback((newState) => {
    ...
    if (newState.rawData !== undefined) {
      console.log('设置 rawData:', newState.rawData);
      setRawData(newState.rawData);
      console.log(newState.rawData.data.projects, 'newState.rawData.data.projects');
      const safeList = newState.rawData?.data?.projects || [];
  ...
  const fetchList = useCallback(async (newParams = {}) => {
    // console.log('开始请求，参数:', newParams);
    const mergedParams = { ...params, ...newParams };
    setParams(mergedParams);
    try {
      const result = await requestList(updateState, '/mock', mergedParams, 'get');
